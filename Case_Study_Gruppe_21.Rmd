...
### Heatmap
Die Heatmap erlaubt es dem Nutzer, sich auf der Deutschlandkarte zu bewegen, um zu sehen, wo sich die am meisten betroffenen Autos befinden. Die Heatmap ist interaktiv und der Nutzer kann nach Belieben hinein- und herauszoomen. Zudem ist es dem Nutzer möglich in einem Fenster die Fahrzeug-ID einzugeben, um damit herauszufinden, ob sein Fahrzeug fehlerhaft ist. Ist dies der Fall erscheint zusätzlich ein Popup auf der Karte mit der exakten Position der Zulassung. Mit einem klick auf das Popup öffnet sich ein kleines Fenster, wo weitere wichtige Informationen zu finden sind, wie 

* Fahrzeug-Herstellernummer
* Produktionsdatum des Fahrzeugs
* Mitteillung, was genau fehlerhaft ist - Fahrzeug, Komponente oder ein Einzelteil
* Datum des Eintritts des Fehlers 
* Fahrleistung des Teils bis zum eintreten des Fehlers 

![Abbildung 1: Heatmap mit Suchfunktion](Zusätzliche_Dateien/Screenshot_Suche.png)

### Produktionsmengen
Im zweiten Tab haben die Fahrzeughersteller der 4 OEM-Werke die Möglichkeit, die Anzahl der im gewählten Zeitraum hergestellten Komponenten oder Einzelteile zu überprüfen. Dabei können sie wählen, ob sie die Grafik für die Komponenten oder die Einzelteile einsehen möchten. Beide Grafiken sind interaktiv gestaltet. Im Balkendiagramm für die Komponenten ist es zudem möglich, den Anteil jeder einzelnen Komponente an der Gesamtmenge der im Werk hergestellten Komponenten einzusehen.

![Abbildung 2: Produktionsmengen - Komponenten](Zusätzliche_Dateien/Screenshot_Produktionsmenge_Komponente.png)

Im Balkendiagramm für die Einzelteile wird **nicht** angezeigt, wie viel jedes Einzelteil zum Gesamtvolumen der Produktion im Werk beiträgt.

![Abbildung 2: Produktionsmengen - Einzelteile](Zusätzliche_Dateien/Screenshot_Produktionsmenge_Einzelteile.png)

### Fahrleistung - Vergleich

Im Tab "Fahrleistung - Vergleich" ermöglichen die Boxplots einen detaillierten Vergleich der Fahrleistung fehlerhafter Fahrzeuge zwischen zwei ausgewählten Gemeinden. Durch die visuelle Darstellung der Daten lassen sich mehrere Interpretationen ableiten: Die Position des Medians (der mittlere Wert) sowie die Länge der Box und die Position der Quartile geben einen Überblick über die zentrale Tendenz und die Streuung der Fahrleistung in jeder Gemeinde. Unterschiede in der Position des Medians zwischen den Gemeinden könnten darauf hinweisen, dass in einer Gemeinde Fahrzeuge tendenziell früher fehlerhaft werden als in der anderen. Darüber hinaus können Ausreißer außerhalb der Boxen identifiziert werden, die potenziell wichtige Einblicke in ungewöhnliche oder problematische Situationen geben könnten. Die Boxlänge sowie die Höhe der Whiskers zeigen die Streuung der Daten und können Aufschluss darüber geben, wie konsistent die Fahrleistung innerhalb jeder Gemeinde ist. Der direkte Vergleich der Boxplots ermöglicht es, signifikante Unterschiede in der Fahrleistung zwischen den Gemeinden zu erkennen, was wichtige Erkenntnisse für die Optimierung von Prozessen und die Verbesserung der Fahrzeugqualität liefern kann.


![Abbildung 3: Vergleich der Fahrleistungen zwischen Gemeinden](Zusätzliche_Dateien/Screenshot_Nutzungsdauer_Vergleich.png)


### Datensatztabelle

Im Tab "Daten" wird eine Übersicht über einen Teil der Rohdaten unserer Datenbank dargestellt. Nutzer können sich hier einen Überblick über die verfügbaren Daten verschaffen und deren Struktur kennenlernen. Die Daten sind in tabellarischer Form präsentiert und können eingesehen sowie durchsucht werden, um ein besseres Verständnis für die verfügbaren Informationen zu erhalten. 

![Abbildung 4: Überischt der Rohdaten](Zusätzliche_Dateien/Screenschot_Daten.png)

### Ergebnissdiskussion
Die Ergebnisse der Shiny-App bieten den OEM Werken und den Fahrzeugbesitzern eine große Auswahl an spezifischen Daten, die optimal auf die jeweilige Suchanfrage zugeschnitten sind. Die Möglichkeit, nach bestimmten Teilen oder nach Orten zu filtern, gibt einen umfassenden Überblick über die zugrunde liegenden Daten. Individuelle Rückschlüsse können so vom Kunden individuell gezogen werden. Über die Filterfunktionen lassen sich auch schnell und einfach gezielte Suchanfragen stellen. Darüber hinaus können die Daten bis auf verschiedene Ebenen (Komponente oder Einzelteile) heruntergebrochen werden. Die Darstellung ist einfach und intuitiv zu verstehen und eignet sich daher für die direkte Präsentation. 

Um die Analyse zu verbessern, könnte eine Funktion implementiert werden, die es ermöglicht, den Anteil jedes Einzelteils an der Gesamtproduktion in jedem Werk anzuzeigen. Dies würde zusätzliche Einblicke in die Verteilung der Produktion geben und den Nutzern helfen, Muster und Trends besser zu erkennen. Des Weiteren wäre es vorteilhaft gewesen, die Gemeinden in der Heatmap stärker zu gruppieren, um möglicherweise herauszufinden, welches Werk am stärksten betroffen ist. Durch eine genauere Gruppierung könnten Zusammenhänge zwischen bestimmten Werken und Produktionsproblemen leichter identifiziert werden.

Die Implementierung dieser Verbesserungen könnte dazu beitragen, die Aussagekraft der Analyse zu erhöhen und den Nutzern eine noch umfassendere und aussagekräftigere Darstellung der Daten zu bieten.
